...
class ResistorLink
  ...
  def printInfo()
    ...
    # puts "#{info}"   # omitted for auto_test by ashbb
  end
...
=begin   # omitted for auto_test by ashbb
...
=end
  ...
  def findUnneededResistors(path)
    ...
    @linkarray.each do |link|
      if link.getUsed() == 0
        $out << link.printInfo()   # edited for auto_test by ashbb
      end
    end
  end
  ...
  # findRoute(@startposition)    # omitted for auto_test by ashbb

#end   # omitted for auto_test by ashbb
